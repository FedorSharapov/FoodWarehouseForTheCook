...
namespace FoodIngredients.Web.Controllers
{
    ...
    {     
        private readonly IIngredientsRpcClient _rpcClient;
        public IngredientsController(IIngredientsRpcClient rpcClient) => _rpcClient = rpcClient;
        ...
        public async Task<IActionResult> Add(IngredientVM ingredient)
        {
            if(ingredient == null || ingredient.Name.Length == 0)
                return BadRequest();

            var response = await _rpcClient.Create(ingredient, UserId);
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Edit(Guid id, IngredientVM ingredient)
        {
            if (id == Guid.Empty || ingredient == null || ingredient.Name.Length == 0)
                return BadRequest();

            var response = await _rpcClient.Update(id, ingredient, UserId);
