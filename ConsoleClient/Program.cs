...
{
    var warehouse = await listWarehouse.Content.ReadFromJsonAsync<WarehouseVM>();
...
List<DishVM> dishes = null;
PrintLn("Список блюд:", ConsoleColor.Blue);
if (listDishes.IsSuccessStatusCode)
{
    dishes = await listDishes.Content.ReadFromJsonAsync<List<DishVM>>();
...
static async Task GetListProducts(HttpClient apiClient)
{
    ...
    {
        var products = await listProducts.Content.ReadFromJsonAsync<List<ProductVM>>();
