...
namespace FW.BusinessLogic.Services
{
    public class CookingDishService : ICookingDishService
    {
        ...
        public async Task<ResponseStatusResult> Cook(Guid dishId, Guid userId, int numPortions)
        {
            ...
            {
                var namesOfMissingIngredients = await GetNamesOfMissingIngredients(quantityProductsForDishInStock, quantityIngredientsDish, userId);
        ...
        private async Task<string> GetNamesOfMissingIngredients(Dictionary<Guid, int> necessaryQuantityProducts, Dictionary<Guid, int> quantityIngredientsDish, Guid userId)
        {
            ...
            var ingredients = await _dbContext.Ingredients
                .Where(i => i.UserId == userId && difference.Contains(i.Id))
                .Select(i => i.Name)
                .ToArrayAsync();
